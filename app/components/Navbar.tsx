...
export default function Navbar() {
  ...
  return (
    <div className="w-full sticky top-0 z-40">
      ...
      {/* Top News Headlines Marquee Ticker Bar (without red badge) */}
      <div className="w-full bg-zinc-950 border-b border-white/10 text-white py-1.5 px-4 flex items-center overflow-hidden relative shadow-inner">
        {/* Scrolling News Ticker Text */}
        <div className="overflow-hidden whitespace-nowrap w-full text-xs font-semibold text-zinc-300">
          ...
          <div className="flex items-center justify-between h-20 md:h-24">
            ...
            <Link href="/" className="flex items-center group shrink-0">
              <Image
                ...
                height={60}
                priority
                className="h-11 sm:h-14 w-auto object-contain transition-all duration-300 group-hover:scale-105 drop-shadow-[0_2px_8px_rgba(0,0,0,0.3)]"
              />
            </Link>

            {/* 5 Navigation Links with Horizontal Underline */}
            <nav className="hidden lg:flex items-center space-x-6 xl:space-x-8 text-sm font-extrabold tracking-wider">
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={`relative py-2 transition-all duration-200 uppercase ${
                      isActive
            ...
            {/* Action Buttons with Hover-Only Silver Shine */}
            <div className="hidden sm:flex items-center space-x-3 shrink-0">
              
              {/* Button 1: APPLY FOR A JOB (Red) */}
              <Link
                href="#apply-job"
                className="group relative overflow-hidden bg-[#e61e2b] hover:bg-red-600 text-white text-xs md:text-sm font-extrabold px-5 py-2.5 rounded-full uppercase tracking-wider transition-all duration-300 shadow-md hover:shadow-lg hover:shadow-red-600/40 hover:scale-105 active:scale-95 flex items-center justify-center"
              >
              ...
              <Link
                href="#apply-guard"
                className="group relative overflow-hidden bg-[#10B981] hover:bg-[#059669] text-white text-xs md:text-sm font-extrabold px-5 py-2.5 rounded-full uppercase tracking-wider transition-all duration-300 shadow-md hover:shadow-lg hover:shadow-emerald-500/40 hover:scale-105 active:scale-95 flex items-center justify-center"
              >
          ...
          <div className="lg:hidden bg-[#2e2b73] border-t border-white/10 px-4 pt-3 pb-6 space-y-3 shadow-2xl">
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`block px-4 py-2.5 text-base font-extrabold uppercase transition-all rounded-lg ${
                    isActive
            ...
            <div className="pt-3 flex flex-col space-y-3">
              ...
              <Link
                ...
                className="group relative overflow-hidden text-center w-full bg-[#e61e2b] text-white font-extrabold px-5 py-3 rounded-full uppercase tracking-wider shadow-md active:scale-95"
              >
                <span className="relative z-10">APPLY FOR A JOB</span>
                <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-full">
                  <div className="absolute top-0 left-0 w-1/2 h-full bg-gradient-to-r from-transparent via-white/60 to-transparent -translate-x-[150%] -skew-x-[25deg] group-hover:animate-button-shine-hover" />
              ...
              <Link
                href="#apply-guard"
                onClick={() => setMobileMenuOpen(false)}
                className="group relative overflow-hidden text-center w-full bg-[#10B981] text-white font-extrabold px-5 py-3 rounded-full uppercase tracking-wider shadow-md active:scale-95"
              >
                <span className="relative z-10">APPLY FOR A GUARD</span>
                <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-full">
                  <div className="absolute top-0 left-0 w-1/2 h-full bg-gradient-to-r from-transparent via-white/60 to-transparent -translate-x-[150%] -skew-x-[25deg] group-hover:animate-button-shine-hover" />
